...
public class PdfService {
	...
	public String buildPDF(String dest, String imgcount, String row, String col, String orientation, List<String> images) {
		...
		try {
			...
			/*
			 * Float docWidth = doc.getStrokeWidth(); Float docHeight = doc.getHeight();
			 * ImageData imgData = ImageDataFactory.create(imgFileDir+images.get(0)); Image
			 * img = new Image(imgData); doc.add(img);
			 */
	...
	public Document attachImages(Document document, String imgcount, String row, String col, String orientation, List<String> images) {
		...
		try {
			...
			float height = PageSize.A4.getHeight() * ((float) heightPercent / 100) - document.getTopMargin()
